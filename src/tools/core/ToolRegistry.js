...
import { Worker } from 'worker_threads'; // Added for Sandbox
 ...
 * - Phase 4: Sandboxed Execution for Dynamic Tools
 */
export class ToolRegistry {
    ...
    _initSandbox() {
        try {
            const workerPath = path.join(__dirname, '../../agent/core/ToolSandboxWorker.js');
            if (fs.existsSync(workerPath)) {
                this.sandboxWorker = new Worker(workerPath);

                this.sandboxWorker.on('message', (msg) => this._handleWorkerMessage(msg));
                this.sandboxWorker.on('error', (err) => {
                    console.error('[ToolRegistry] Sandbox Worker Crashed:', err);
                    this.sandboxWorker = null;
                    // Respawn logic could go here
                });
                console.log('[ToolRegistry] 🛡️ Sandbox Worker Ready');
            } else {
                console.warn('[ToolRegistry] Sandbox Worker file not found.');
            }
        } catch (e) {
            console.error('[ToolRegistry] Failed to init sandbox:', e);
        }
    }

    async _handleWorkerMessage(msg) {
        if (msg.type === 'AGENT_CALL') {
            // Handle RPC from Worker
            // msg: { type: 'AGENT_CALL', method: 'bot.chat', args: ['hello'] }
            const { method, args, execId } = msg;
            // We usually don't have execId in the proxy yet, but let's assume fire-and-forget for now
            // or we need a 2-way RPC. The current worker implementation is simple.

            if (method === 'bot.chat') {
                this.agent.bot.chat(args[0]);
            }
            // Add more RPC handlers here as needed
            return;
        }

        // Handle Execution Results
        if (msg.execId !== undefined && this.pendingExecutions.has(msg.execId)) {
            const resolver = this.pendingExecutions.get(msg.execId);
            if (msg.status === 'SUCCESS') {
                resolver.resolve({ success: true, result: msg.result });
            } else {
                resolver.resolve({ success: false, error: msg.error });
            }
            this.pendingExecutions.delete(msg.execId);
        }
    }
    ...
    async discoverSkills() {
        ...
        try {
            const skillsPath = path.join(process.cwd(), 'src/skills/library');
            const dynamicToolsPath = path.join(process.cwd(), 'src/tools/dynamic');

            const pathsToScan = [skillsPath];
            if (fs.existsSync(dynamicToolsPath)) {
                pathsToScan.push(dynamicToolsPath);
            } else {
                // Ensure it exists for future use
                fs.mkdirSync(dynamicToolsPath, { recursive: true });
            }
            ...
            for (const dirPath of pathsToScan) {
                if (!fs.existsSync(dirPath)) {
                    console.warn('[ToolRegistry] Directory not found:', dirPath);
                    continue;
                }

                const files = getFiles(dirPath).filter(f => f.endsWith('.js'));

                for (const file of files) {
                    try {
                        await this._loadSkill(file);
                    } catch (error) {
    ...
    _extractMetadata(skillModule, skillPath) {
        ...
        return {
            ...
            execute: skill,
            path: skillPath,
            isDynamic: skillPath.includes('dynamic') // Flag for sandbox
        };
    }
    ...
    async executeSkill(name, params = {}) {
        ...
        try {
            ...
            let result;
            if (skill.isDynamic && this.sandboxWorker) {
                // Run in Sandbox
                result = await this.executeSandboxedSkill(name, skill.path, params);
            } else {
                // Run locally
                result = await skill.execute(this.agent, params);
            }
    ...
    async executeSandboxedSkill(name, filePath, params) {
        if (!this.sandboxWorker) {
            return { success: false, error: 'Sandbox not available' };
        }

        const execId = this.execIdCounter++;
        return new Promise((resolve) => {
            ...
            }, 10000); // 10s Limit for complex tools
    ...
    shutdown() {
        if (this.sandboxWorker) {
            try {
                ...
                console.error('[ToolRegistry] Error terminating sandbox:', e);
            }
            this.sandboxWorker = null;
        }
    }
