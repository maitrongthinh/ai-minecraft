...
export const actionsList = [
    ...
    },
    {
        ...
        params: {
            'blueprint_name': { type: 'string', description: 'The name of the blueprint to build.' }
        },
        perform: async function (agent, blueprint_name) {
            const blueprint = agent.blueprintManager.getBlueprint(blueprint_name);
            if (!blueprint) {
                return `Blueprint "${blueprint_name}" not found. Available: ${agent.blueprintManager.listBlueprints().join(', ')}`;
            }
            await agent.npc.setGoal(blueprint_name);
            return `Started building "${blueprint_name}".`;
        }
    },
];
