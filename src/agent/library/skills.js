...
export async function collectBlock(bot, blockType, num = 1, exclude = []) {
    /**
...
export async function placeBlock(bot, blockType, x, y, z, faceStr = 'top', shouldJump = false) {
    /**
...
export async function equip(bot, itemName, destination = 'hand') {
    /**
...
export async function goToNearestBlock(bot, blockType, minDistance = 2, range = 64) {
    /**
...
export async function moveAway(bot, distance) {
    ...
    log(bot, `Moved away from ${pos.floor()} to ${new_pos.floor()}.`);
    return true;
}
...
export async function gather_wood(bot, num = 1) {
    /**
     * Gather wood (logs) from the nearest tree.
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @param {number} num, number of logs to collect.
     * @returns {Promise<boolean>} true if wood was gathered.
     */
    log(bot, `Gathering ${num} wood...`);
    ...
    for (const logType of logTypes) {
        // limit search to see if any exist first to save time? 
        // collectBlock already searches.
        const success = await collectBlock(bot, logType, num);
        if (success) return true;
    }
    log(bot, 'Could not find any trees/logs nearby.');
    return false;
}

export async function find_shelter(bot) {
    /**
     * Find a safe spot (shelter) or create one.
     * @param {MinecraftBot} bot
     */
    log(bot, 'Looking for shelter...');
    // 1. Look for a covered area (cave, house)
    // Simple check: is there a block above head?
    const position = bot.entity.position;
    if (bot.blockAt(position.offset(0, 2, 0))?.name !== 'air') {
        log(bot, 'Already under shelter.');
        return true;
    }

    // 2. Look for existing structure nearby (TODO: robust structure detection)

    // 3. Dig a hole/burrow
    log(bot, 'Digging emergency shelter...');
    // Dig 3 blocks down
    for (let i = 0; i < 3; i++) {
        await breakBlockAt(bot, position.x, position.y - i, position.z);
        await new Promise(r => setTimeout(r, 500));
    }
    // Jump in? Actually we are standing on it, so we fall in.
    // Cover top?
    return true;
}
